...
namespace gm
{
    ...
    int compare(double x, double y, double precision)   //比较两个浮点数
    {
    ...
    bool isPointOnLineString(const GeoRawPoint& pt, GeoLineString* line, double precision)
    {
    ...
    bool isPointInLinearRing(const GeoRawPoint& pt, GeoLinearRing* pRing, double precision)
    {
        ...
        for (int i = 0, j = pointsCount - 1; i < pointsCount; j = i++) {
            if (isPointOnLine(pt, ring[i], ring[j]))
                return true;
            if (compare(ring[i].y, ring[j].y) == 0)
                continue;
